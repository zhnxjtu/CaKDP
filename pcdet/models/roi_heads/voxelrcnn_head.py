...
class VoxelRCNNHead(RoIHeadTemplate):
    ...
    def roi_grid_pool(self, batch_dict):
        ...
        for k, src_name in enumerate(self.pool_cfg.FEATURES_SOURCE):
            ...
            if batch_dict.get('re_run_tea_flag', None):
                cur_stride = batch_dict['multi_scale_3d_strides_tea'][src_name]
                cur_sp_tensors = batch_dict['multi_scale_3d_features_tea'][src_name]

                if with_vf_transform:
                    cur_sp_tensors = batch_dict['multi_scale_3d_features_post_tea'][src_name]
                else:
                    cur_sp_tensors = batch_dict['multi_scale_3d_features_tea'][src_name]
    ...
    def forward(self, batch_dict):
        ...
        if not self.training and not batch_dict.get('re_run_tea_flag', None):
        ...
        if not self.training and batch_dict.get('re_run_tea_flag', None):
            batch_dict['rcnn_cls_stu_to_tea'] = rcnn_cls
            batch_dict['rcnn_reg_stu_to_tea'] = rcnn_reg

            # batch_cls_preds, batch_box_preds = self.generate_predicted_boxes(
            #     batch_size=batch_dict['batch_size'], rois=batch_dict['rois'], cls_preds=rcnn_cls, box_preds=rcnn_reg
            # )

            # batch_dict['rcnn_reg_stu_to_tea'] = batch_box_preds
            # batch_cls_preds_stu_ti_tea, batch_box_preds_stu_ti_tea = self.generate_predicted_boxes(
            #     batch_size=batch_dict['batch_size'], rois=batch_dict['rois'],
            #     cls_preds=rcnn_cls, box_preds=rcnn_reg
            # )
            # batch_dict['batch_cls_preds_stu_to_tea'] = batch_cls_preds_stu_ti_tea
            # batch_dict['batch_box_preds_stu_to_tea'] = batch_box_preds_stu_ti_tea
            # batch_dict['cls_preds_normalized'] = False
