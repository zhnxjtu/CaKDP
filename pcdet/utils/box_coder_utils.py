...
class ResidualCoder(object):
    ...
    # TODO: add
    def encode_torch_coords(self, boxes, anchors):
        """
        Args:
            boxes: (N, 7 + C) [x, y, z, dx, dy, dz, heading, ...]
            anchors: (N, 7 + C) [x, y, z, dx, dy, dz, heading or *[cos, sin], ...]
        ...
        coord_a_min_x, coord_a_min_y = xa - dxa/2.0, ya - dya/2.0
        coord_a_max_x, coord_a_max_y = xa + dxa/2.0, ya + dya/2.0
        coord_g_min_x, coord_g_min_y = xg - dxg/2.0, yg - dyg/2.0
        coord_g_max_x, coord_g_max_y = xg + dxg/2.0, yg + dyg/2.0

        left_bottom = torch.sqrt((coord_a_min_x - coord_g_min_x) ** 2 + (coord_a_min_y - coord_g_min_y) ** 2) / diagonal
        right_bottom = torch.sqrt((coord_a_min_x - coord_g_min_x) ** 2 + (coord_a_max_y - coord_g_max_y) ** 2) / diagonal
        right_top = torch.sqrt((coord_a_max_x - coord_g_max_x) ** 2 + (coord_a_max_y - coord_g_max_y) ** 2) / diagonal
        left_top = torch.sqrt((coord_a_max_x - coord_g_max_x) ** 2 + (coord_a_min_y - coord_g_min_y) ** 2) / diagonal

        cts = [g - a for g, a in zip(cgs, cas)]
        return torch.cat([left_bottom, right_bottom, right_top, left_top, *cts], dim=-1)
