...
from pcdet.models.dense_heads import CenterHead, AnchorHeadTemplate, PointHeadBox
...
def adjust_batch_info_teacher(batch):
    ...
    if cfg.KD.get('MASK', None):
        ...
        if cfg.KD.MASK.get('DIF_MASK', None):
            teacher_pred_flag = True
...
def add_teacher_pred_to_batch(teacher_model, batch, pred_dicts=None):
    if cfg.KD.get('FEATURE_KD', None) and cfg.KD.FEATURE_KD.ENABLED:
        ...
        if feature_name == 'multi_scale_3d_features':
            batch[feature_name + '_tea'] = batch[feature_name]
            batch['encoded_spconv_tensor_tea'] = batch['encoded_spconv_tensor']
            batch['spatial_features_2d_tea'] = batch['spatial_features_2d'].detach()
        elif feature_name == 'point_features':
            batch[feature_name + '_tea'] = batch[feature_name]
            batch['point_coords_tea'] = batch['point_coords']
        elif feature_name == 'cls_preds':
            batch['batch_cls_preds_tea'] = batch['batch_cls_preds'].detach()
            batch['batch_box_preds_tea'] = batch['batch_box_preds'].detach()
            ...
            if batch.get('multi_scale_3d_strides', None):
                batch['multi_scale_3d_strides_tea'] = batch['multi_scale_3d_strides']
                batch['multi_scale_3d_features_tea'] = batch['multi_scale_3d_features']
            if batch.get('multi_scale_3d_features_post', None):
                batch['multi_scale_3d_features_post_tea'] = batch['multi_scale_3d_features_post']
            # batch['batch_' + feature_name + '_tea'] = teacher_model.roi_head.forward_ret_dict['rcnn_cls']
        else:
    ...
    if batch.get('teacher_pred_flag', None):
        ...
        elif isinstance(teacher_model.point_head, PointHeadBox) and (teacher_model.dense_head is None):
            batch['batch_cls_preds_tea'] = teacher_model.point_head.forward_ret_dict['batch_cls_preds']
...
def forward(model, teacher_model, batch, optimizer, extra_optim, optim_cfg, load_data_to_gpu, **kwargs):
    ...
    with torch.no_grad():
        ...
        batch['teacher_model'] = teacher_model


        # for u,v in batch.items():
        #     print(u)
        #
        # print(batch['batch_cls_preds_tea'].shape)
        # print(batch['batch_cls_preds_tea_densehead'].shape)
        #
        # exit()
