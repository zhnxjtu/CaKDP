...
class PointNet2MSG(nn.Module):
    def __init__(self, model_cfg, input_channels, **kwargs):
        ...
        if model_cfg.get('WIDTH', None):
            self.model_cfg.SA_CONFIG.MLPS = (np.array(self.model_cfg.SA_CONFIG.MLPS, dtype=np.int32) * model_cfg.WIDTH).astype(int).tolist()
            self.model_cfg.FP_MLPS = (np.array(self.model_cfg.FP_MLPS, dtype=np.int32) * model_cfg.WIDTH).astype(int).tolist()
    ...
    def forward(self, batch_dict):
        ...
        # print(batch_dict['point_features'].shape)
        # print(batch_dict['point_coords'])
        # exit()
        return batch_dict
