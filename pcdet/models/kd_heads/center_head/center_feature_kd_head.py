...
class CenterFeatureKDHead(KDHeadTemplate):
    ...
    def get_feature_kd_loss_point(self, batch_dict, loss_cfg):
        ...
        select_mask = batch_dict['mask_use_item'].view(-1, 1)
        cls_preds_selected_reverse_tea = batch_dict['cls_preds_selected_reverse_tea']
        cls_preds_selected_reverse_stu = batch_dict['cls_preds_selected_reverse_stu']

        kd_feature_loss = loss_cfg.weight_reverse * \
                          (self.kd_feature_loss_func(cls_preds_selected_reverse_stu,
                                                     cls_preds_selected_reverse_tea).sum()/ torch.clamp(
